...
m2=leaflet(l2) %>% 
    ...
    addCircleMarkers(color=~pal(l2$DEP_DELAY_NEW),label = ~format(l2$DEP_DELAY_NEW))
m2
```
...
m3=leaflet(a) %>% 
    addProviderTiles(providers$CartoDB.Positron) %>% 
    addCircleMarkers(lng=a$LONGITUDE, lat=a$LATITUDE,color=~pal2(DEP_DELAY_NEW),label=~format(a$DEP_DELAY_NEW))
m3
```
...
#ggplot(g2a,aes(x = perDelay, y = Flow)) +geom_point() + labs(x="Punctuality rate
#", y="Passager flow")
```
