...
You can view the airport list by use "unique"
```{r}
...
```{r}
airport_joined=left_join(codeunique, AirportData, by = c("Code" = "AIRPORT"))

```

View the Location Map Use "Leaflet"
```{r}
leaflet() %>% addProviderTiles(providers$OpenStreetMap) %>% addMarkers(airport_joined$LONGITUDE,airport_joined$LATITUDE,popup=airport_joined$DISPLAY_AIRPORT_NAME)
```
