...
```{r}
leaf1=leaflet(airport_joined) %>% setView(-96, 37.8, 3) %>%    addProviderTiles(providers$OpenStreetMap) %>% addMarkers(lng=~LONGITUDE,lat=~LATITUDE,popup=airport_joined$DISPLAY_AIRPORT_NAME,clusterOptions = markerClusterOptions())
leaf1
```
...
```{r}
leaf2=leaflet(l2) %>% setView(-96, 37.8, 4) %>% addProviderTiles(providers$CartoDB.Positron) %>% addCircleMarkers(color=~pal(l2$DEP_DELAY_NEW))
leaf2
```
...
pal2=colorBin("YlOrRd",domain = a$DEP_DELAY_NEW)
```



```{r}
leaflet(a) %>% addProviderTiles(providers$CartoDB.Positron) %>% addCircleMarkers(lng=a$LONGITUDE, lat=a$LATITUDE,color=~pal2(a$DEP_DELAY_NEW))
```
