...
```{r}
leaflet(airport_joined) %>% setView(-96, 37.8, 3) %>%    addProviderTiles(providers$CartoDB.PositronNoLabels) %>% addMarkers(lng=~LONGITUDE,lat=~LATITUDE,popup=airport_joined$DISPLAY_AIRPORT_NAME,clusterOptions = markerClusterOptions())
```
