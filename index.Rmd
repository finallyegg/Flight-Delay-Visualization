...
pal2=colorQuantile("Blues", finished$DEP_DELAY_NEW, n = 5)
```
...
m3=leaflet(a) %>% 
    addProviderTiles(providers$CartoDB.Positron) %>% 
    addCircleMarkers(lng=a$LONGITUDE, lat=a$LATITUDE,color=~pal2(a$DEP_DELAY_NEW),popup = popuplabels2,radius = ~sqrt(a$Flow)*0.01)
m3
```
...
Show tables, plots, etc. and describe the.
